...
const openSignatureButton = document.querySelector("[data-open-signature]");
const closeSignatureButton = document.querySelector("[data-close-signature]");
...
function sessionExpiresAt(session = dashboardSession) {
  if (Number.isFinite(Number(session?.expires_at))) {
    return Number(session.expires_at) * 1000;
  }

  try {
    let encodedPayload = session.access_token.split(".")[1].replace(/-/g, "+").replace(/_/g, "/");
...
function scheduleSessionRefresh() {
  window.clearTimeout(sessionRefreshTimer);
  if (!dashboardSession?.refresh_token) return;

  const refreshIn = Math.max(5000, sessionExpiresAt() - Date.now() - refreshMarginMs);
  sessionRefreshTimer = window.setTimeout(() => {
    refreshSession().catch(() => {
      clearSession();
      showLogin("Sua sessao foi revogada. Entre novamente.");
    });
  }, refreshIn);
}
...
async function refreshSession() {
  ...
  if (!dashboardSession?.refresh_token) throw new Error("Sessao sem token de renovacao.");
  ...
  sessionRefreshPromise = (async () => {
    const response = await fetch(`${baseUrl()}/auth/v1/token?grant_type=refresh_token`, {
      ...
      body: JSON.stringify({ refresh_token: dashboardSession.refresh_token }),
    });

    if (!response.ok) throw new Error("Nao foi possivel renovar a sessao.");
...
async function ensureFreshSession(force = false) {
  if (!dashboardSession) throw new Error("Sessao indisponivel.");
  const isExpiring = sessionExpiresAt() <= Date.now() + refreshMarginMs;
  if (force || isExpiring) await refreshSession();
}
...
function setSignatureMode(isOpen) {
  dashboardApp?.classList.toggle("is-signature-mode", isOpen);
  if (signatureWorkspace) signatureWorkspace.hidden = !isOpen;
}
...
async function restoreSession() {
  ...
  try {
    ...
    clearSession();
    showLogin("Nao foi possivel restaurar sua sessao. Entre novamente.");
  } finally {
    await hideLoading();
  }
}
